...
- **원인 / 현상**: 선이 사라지면 방향 판단 실패 → 무작위 움직임  
- **해결 조치**: 반대 방향으로 후진 → 라인 재인식 시 전진  
- **결과**: 미검출 상황에서도 경로 복귀 가능

---

### 🔹 [④] 수동/자율 모드 전환 지연

- **원인 / 현상**: `CH7` 신호 충돌 → 후진 상태가 유지됨  
- **해결 조치**: `PinChangeInterrupt` 기반 신호 처리 + 상태 초기화  
- **결과**: 모드 전환이 즉시 반영됨
...
<img src="linetracing_gif.gif" width="480"/>
<p align="center"><i>Zigzag 보정이 적용된 직선 추종 주행</i></p>
...
<img src="images/camera_position_fix.jpg" width="480"/>
